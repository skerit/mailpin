...
Connection.setMethod(function processSmtpPayload(data) {
	...
	// params = Blast.Bound.String.splitOnce(data, ' ');
 ...
 * @version  0.1.0
 */
Connection.setMethod(function doCommand(command, data, callback) {

	if (!callback) {
		callback = Fn.thrower;
	}

	console.log('Do command', command, 'with', data)

	switch (command) {
		case 'HELO':
		case 'EHLO':
		case 'LHLO':
			this.opening_cmd = command;
			break;
	}
	...
	if (this.server.options.lmtp) {
		switch (command) {
			case 'HELO':
			case 'EHLO':
				this.send(500, 'Error: ' + command + ' not allowed in LMTP server');
				return;
	...
	if (command == 'EHLO') {
		this.send(250, ['Nice to meet you', 'PIPELINING']);
	}

	if (this.server.supports(command)) {
		let method = 'handle' + command;
